...
;(async () => {
  ...
  const githubUsernameQ = await question(`Enter your github username${githubUsername ? ` (${githubUsername})` : ``}: `)
  githubUsername = githubUsernameQ || githubUsername
  if (!githubUsername?.length > 0) exitError("Github username is required")

  const githubRepoQ = await question(`Enter your forked repo name${githubRepo ? ` (${githubRepo})` : ``}: `)
  githubRepo = githubRepoQ || githubRepo
  if (!githubRepo?.length > 0) exitError("Repo name is required")

  const getBot = await axios.get(`https://api.telegram.org/bot${accessToken}/getMe`).catch(exitError)
