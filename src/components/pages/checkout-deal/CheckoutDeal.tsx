...
const CheckoutDeal = () => {
  ...
  function handleChangeAmountToReceive(event: any) {
    ...
    if (String(order.orderType) === OrderType[OrderType.SELL]) {
      deal.amount = Number(value)
      setAmountToPay(Number((value * deal.price).toFixed(2)))
    } else if (String(order.orderType) === OrderType[OrderType.BUY]) {
      deal.amount = Number((value / deal.price).toFixed(2))
    ...
    <div>
      ...
      <div>
        ...
        <div>
          <TextField
            id="pay"
            label={"I want to pay " + (String(order.orderType) === OrderType[OrderType.SELL] ? order.currency : "TON")}
            variant="outlined"
            type="number"
            value={amountToPay}
            error={!!errorAmount}
      ...
      <div>
        ...
        <Link to={PATH_ORDER_CONFIRMATION} state={deal}>
          Buy TON
        </Link>
      </div>
    </div>
